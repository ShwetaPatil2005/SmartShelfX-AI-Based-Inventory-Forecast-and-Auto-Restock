...
const ProductEntry = () => {
    ...
    // In ProductEntry.jsx
    const setVendors = () => {
    // Ensure this matches the new @GetMapping("/invent/users/role/Vendor")
    getUsersByRole('Vendor').then((response) => {
        setVendorList(response.data); // response.data will be ["Vendor1", "Vendor2"]
    });
}
...
<div className="form-group">
    ...
    >
        {/* Explicitly set value="" so your validation works correctly */}
        <option value="">---</option> 
        {
            vendorList && vendorList.length > 0 ? (
                vendorList.map((user, index) => (
                    // Using user as both key and value since it's a unique string (username)
    ...
    {errors.vendorId && <p className="text-danger">{errors.vendorId}</p>}
</div>
